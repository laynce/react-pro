...
function Content(props) {
  ...
  useEffect(() => { // 组件染后执行操作
    console.log(count, '更新了已经')
    var a = 10
     let timer = setInterval(() => {
      console.log('timer', 111, a++, timer && timer)
    }, 2000)

     return () => { // useEffect 可以在组件渲染后实现各种不同的副作用。有些副作用可能需要清除，所以需要返回一个函数
        // seEffect 会在调用一个新的 effect 之前对前一个 effect 进行清理
       clearInterval(timer)
       console.log('清理前一个timer', timer, 111)
     }
  })
